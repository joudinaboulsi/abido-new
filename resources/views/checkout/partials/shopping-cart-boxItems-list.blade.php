	...
	@foreach($box->boxItem as $item)
		...
		<td>
			<div class="shopping_cart_product_info">
				...
				<!-- Remove Link  -->
				<a href="#" onclick="removeBoxItem({{$item->id}},{{$box->id}})" class="link_colors shopping_cart_remove_link">Remove</a>
			</div>
		</td>
		...
		<td>
			<small>Qty</small>
			<input type="number" value="{{$item->quantity}}" id="order_item_box{{$item->id}}" onchange="editOrderItemBox({{$item->id}})" class="shopping_cart_item_box form-control" data-variant="{{ $item->id }}">
		</td>
