...
function remove_Product_FromWishList($user_id, $variant_id)
{
    $url =
        env('API_URL') .
        '/api/remove_Product_FromWishList/' .
        $user_id .
        '/' .
        $variant_id;
...
{
    $url =
        env('API_URL') . '/api/add_To_WishList/' . $user_id . '/' . $variant_id;
...
function getBoxData($box_id)
{
    $url = env('API_URL') . '/api/boxes/' . $box_id;

    return $response = httpGetRequest($url);
}
function getBoxDataItem($box_id)
{
    $url = env('API_URL') . '/api/boxes/items/' . $box_id;
...
{
    $url =
        env('API_URL') . '/api/variants/is-valid/' . $variant_id . '/' . $qty;
...
function addToCartBoxApi($user_id, $box_id)
{
    $url = env('API_URL') . '/api/add-to-cart-box/' . $user_id . '/' . $box_id;

    return $response = httpGetRequest($url);
}

function addToCartBoxItemApi($order_box_id, $variant_id, $qty)
{
    $url =
        env('API_URL') .
        '/api/add-to-cart-box-item/' .
        $order_box_id .
        '/' .
        $variant_id .
        '/' .
...
function updateCartQtyBoxApi($user_id, $variant_id, $qty)
{
    $url =
        env('API_URL') .
        '/api/edit-cart-qty-box/' .
        $user_id .
        '/' .
        $variant_id .
        '/' .
...
function removeItemApi($user_id, $item_id)
{
    $url = env('API_URL') . '/api/remove-item/' . $user_id . '/' . $item_id;

    return $response = httpGetRequest($url);
}

function removeBoxApi($user_id, $box_id)
{
    $url = env('API_URL') . '/api/remove-box/' . $user_id . '/' . $box_id;

    return $response = httpGetRequest($url);
}

function removeBoxItemApi($user_id, $box_id, $item_id)
{
    $url =
        env('API_URL') .
        '/api/remove-box-item/' .
...
function updateStatusBox($box_id)
{
    $url = env('API_URL') . '/api/boxes/status/' . $box_id;
...
{
    $url =
        env('API_URL') . '/api/apply-promo-code/' . $promoCode . '/' . $userId;

    return $response = httpGetRequest($url);
}

function qtyToRemoveFromStockApi($userId, $variantId)
{
    $url =
        env('API_URL') .
        '/api/qty-ToRemoveFromStock/' .
        $userId .
        '/' .
        $variantId;
...
{
    $url =
        env('API_URL') . '/api/delete-address/' . $address_id . '/' . $user_id;
