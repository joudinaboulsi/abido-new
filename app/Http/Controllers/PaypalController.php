...
class PaypalController extends Controller
{
    public function __construct()
    {
        ...
        $this->_api_context->setConfig([
            'mode' => env(
                'PAYPAL_MODE',
                app('settings')->checkout->paypal_mode
            ),
    ...
    public function payWithpaypal(Request $request)
    {
        ...
        foreach ($cart->orderItems as $orderItem) {
            $item = new Item();
            $item
                ->setName($orderItem->product_title)
        ...
        $details = new Details();
        $details
            ->setShipping($cart->shipping_fees)
        ...
        $redirect_urls
            ->setReturnUrl(route('paypalSuccess'))
        ...
        try {
            ...
            if (\Config::get('app.debug')) {
                ...
                \Session::put(
                    'error',
                    'Some error occur, sorry for inconvenient'
                );
                return \Redirect::route('paywithpaypal');
            }
        }
